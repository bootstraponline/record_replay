...
class Request {
  constructor(request) {
    ...
    if (contentLength) {
      this.bodySize = _parseInt$2(contentLength, 10);
    } else {
...
class Response {
  constructor(response) {
    ...
    if (contentLength) {
      this.content.size = _parseInt$2(contentLength, 10);
    } else {
...
class Persister {
  ...
  static get name() {
    utils.assert('Must override the static `name` getter.');
  }
  ...
  async persist() {
    ...
    for (const [recordingId, {
      ...
      for (const request of requests) {
        ...
        this.assert(`Cannot persist response for [${entry.request.method}] ${entry.request.url} because the status code was ${entry.response.status} and \`recordFailedRequests\` is \`false\``, request.response.ok || request.config.recordFailedRequests);
        /*
...
//# sourceMappingURL=pollyjs-persister.js.map
